...
def create_gif(filename, env, q_network):
    gif_frames = []
    done = False
    state, _ = env.reset()
    frame = env.render()
    gif_frames.append(PIL.Image.fromarray(frame))
    while not done:
    ...
    gif_frames[0].save(filename, save_all=True, append_images=gif_frames[1:], loop=0, optimize=False)